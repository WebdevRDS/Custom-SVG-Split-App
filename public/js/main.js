...
$(document).ready(function() {
  ...
  var GetFileObjectFromURL = function (filePathOrUrl, convertBlob) {
    GetFileBlobUsingURL(filePathOrUrl, function (blob) {
      convertBlob(blobToFile(blob, "713500110101.svg"));
    });
  };

  $("button").on("click", function () {
    $.get("http://localhost:9000/svg/list", function (fileNames) {
      fileNames.forEach(fileName => {
        GetFileObjectFromURL(`./svg/${fileName}`, function (fileObject) {
          readfiles([fileObject], fileName);
        });
      })
    });
  ...
  function readfiles(files, fileName) {
  ...
  function parseFile(file, fileName) {
    if (tests.filereader === true && acceptedTypes[file.type] === true) {
      ...
      reader.onload = function (event) {
        ...
        newZip.forEach((z, i) => {
          console.log(i);
          const url = `data:application/zip;base64,${z.generate()}`;
          const hiddenIFrameID = "hiddenDownloader" + fileName + "_" + i;
